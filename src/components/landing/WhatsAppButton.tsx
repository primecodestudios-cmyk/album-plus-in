  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("send-whatsapp", {
        body: {
          number,
          message: "Hi AlplumPlus Team, I need help with ...",
          category: "manual",
        },
      });

      // Regardless of API result, open WhatsApp chat as fallback
      const waUrl = `https://api.whatsapp.com/send?phone=${number}&text=${encodeURIComponent("Hi AlplumPlus Team, I need help with ...")}`;
